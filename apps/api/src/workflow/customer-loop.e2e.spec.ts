...
async function createResumeFixtureDocx(): Promise<{ dir: string; filePath: string }> {
  const dir = await mkdtemp(path.join(os.tmpdir(), 'ttr-real-loop-'));
  const filePath = path.join(dir, 'strong-fit-resume.docx');
  ...
  const lines = [
    ...
    '',
    'Example SaaS | Support Operations Program Owner | 2024 - Present',
