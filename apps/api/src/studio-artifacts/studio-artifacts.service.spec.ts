...
describe('StudioArtifactsService (unit): resumeResult contract', () => {
  ...
  it('returns persisted-only state for assessment-backed Studio routes without baseline hydration or regeneration', async () => {
      ...
      findOne: jest.fn().mockResolvedValue({
        ...
        resumeResponseBody: {
          ...
          preview: {
            resume: {
              heading: { name: 'Alex Candidate', contactLine: 'alex@example.com' },
              summary: 'Persisted resume preview.',
              experience: [{ company: 'Co', roleTitle: 'Role', bullets: ['Did work.'] }],
            },
          },
        },
    ...
    jest.spyOn(service as any, 'computeResumeInputsHash').mockReturnValue('stored-hash');
    jest.spyOn(service as any, 'computeCoverLetterInputsHash').mockReturnValue('stored-hash');
  ...
  it('returns an empty canonical payload for assessment-backed Studio routes when no artifact row exists', async () => {
    const studioArtifactRepository = {
    ...
    const baselineResumeV2BackfillService = {
      backfillLatestIfMissing: jest.fn(),
    } as any;
    const resumeService = {
      generateResume: jest.fn(),
    } as any;
  ...
  it('hydrates a persisted resume artifact using the analysis binding stored on the artifact row', async () => {
    const studioArtifactRepository = {
      findOne: jest.fn().mockResolvedValue({
        id: 'artifact-3',
        ...
        resumeStatus: StudioArtifactLifecycleStatus.COMPLETED,
        resumeInputsHash: 'resume-stored-hash',
        resumeResponseBody: {
          status: 'success',
          generationStatus: 'success',
          exportReady: true,
          ...
          preview: {
            resume: {
              heading: { name: 'Alex Candidate', contactLine: 'alex@example.com' },
              summary: 'Persisted resume summary.',
        ...
        resumeFailedAt: null,
        resumeMetadata: { analysisId: 'analysis-old' },
        coverLetterStatus: StudioArtifactLifecycleStatus.COMPLETED,
        coverLetterInputsHash: 'cover-hash',
        coverLetterResponseBody: {
          status: 'success',
          ...
          exports: { docx: true, pdf: true },
          qualityGate: { status: 'pass', reasons: [] },
          preview: { coverLetter: { paragraphs: ['Dear Hiring Team,', 'Persisted cover letter.'] } },
        },
        ...
        coverLetterFailedAt: null,
        coverLetterMetadata: { analysisId: 'analysis-current' },
      } as any),
    } as any;
    ...
    const fitAssessmentRepository = {
      findOne: jest.fn().mockImplementation(async (query: any) => {
        const analysisId = query?.where?.id;
        if (analysisId === 'analysis-old') {
          return { overallScore: 88, inputsHash: 'resume-old-inputs' };
        }
        return { overallScore: 91, inputsHash: 'resume-current-inputs' };
      }),
    } as any;
    const baselineResumeV2BackfillService = {
      backfillLatestIfMissing: jest.fn().mockResolvedValue(null),
    ...
    jest.spyOn(service as any, 'computeResumeInputsHash').mockImplementation((input: any) =>
      input.assessmentInputsHash === 'resume-old-inputs' ? 'resume-stored-hash' : 'resume-current-hash',
    );
    jest.spyOn(service as any, 'computeCoverLetterInputsHash').mockReturnValue('cover-hash');
    ...
    expect(state.resume?.metadata?.analysisId).toBe('analysis-old');
...
describe('StudioArtifactsService (unit): canonical generated artifact persistence', () => {
  ...
  it('returns persisted-only failed artifact state during Studio readState and does not self-repair in GET', async () => {
    const stored: any = {
      ...
      resumeResponseBody: {
        status: 'failed',
        error: { code: 'legacy_failed' },
      },
      resumeContent: null,
      resumeFailureCode: 'legacy_failed',
      resumeFailureMessage: 'previous generation failed',
      resumeGenerationStartedAt: null,
      resumeGeneratedAt: null,
      resumeFailedAt: new Date('2026-06-03T00:00:30.000Z'),
      resumeMetadata: { analysisId: 'analysis-1' },
    ...
    const queryBuilder: any = {
      ...
      execute: jest.fn().mockImplementation(async () => {
        if (pendingPatch) {
          Object.assign(stored, pendingPatch);
          pendingPatch = null;
        }
        return {};
      }),
  ...
  it('regenerates a current failed unusable resume artifact on Studio entry when retryAllowed is true and canonical inputs are now valid', async () => {
    ...
    const baselineRepository = {
      createQueryBuilder: jest.fn().mockReturnValue({
        ...
        getRawMany: jest.fn().mockResolvedValue([
          {
            ...
            sections_sectionType: 'SUMMARY',
            sections_title: 'Summary',
            sections_content: 'Customer operations leader focused on measurable improvements and execution cadence.',
            sections_includePolicy: 'OPTIONAL',
            sections_order: 0,
            sections_createdAt: new Date(),
            sections_updatedAt: new Date(),
            ...
            parsedRecords_parsedJson: {
              experience: [
                ...
                {
                  company: 'Parsed Co',
                  role_title: 'Support Operations Lead',
                  start_date: 'Jan 2016',
                  end_date: 'Dec 2017',
                  details_text: 'Owned support playbooks and routing.',
                },
                {
                  company: 'Parsed Co',
                  role_title: 'Support Coordinator',
                  start_date: 'Jan 2014',
                  end_date: 'Dec 2015',
                  details_text: 'Coordinated escalations and reporting.',
                },
              ],
            },
            parsedRecords_resumeV2Json: {
        ...
        getOne: jest.fn().mockResolvedValue({ overallScore: 42, inputsHash: 'inputs-1' }),
      }),
    } as any;
    const baselineVersionRepository = { findOne: jest.fn().mockResolvedValue({ id: 'base-version-1', baselineId: 'base-1', hash: 'hash-v1' }) } as any;
    const jobRepository = { findOne: jest.fn().mockResolvedValue({ id: 'job-1', title: 'Role', company: 'Co' }) } as any;
        ...
        stored.resumeResponseBody = {
          ...
          preview: {
            resume: {
              heading: { name: 'Alex Candidate', contactLine: 'alex@example.com' },
              summary: 'Recovered resume summary.',
              experience: [{ company: 'Parsed Co', roleTitle: 'Director of Support Operations', bullets: ['Led support operations and exec updates.'] }],
              education: [],
              competencies: [],
            },
          },
            ...
            parsedRecords_parsedJson: {
              experience: [
                ...
                },
                {
                  ...
                  start_date: 'Jan 2017',
                  end_date: 'Dec 2019',
                  details_text: 'Improved service delivery.',
                },
              ],
            },
            parsedRecords_resumeV2Json: {
        ...
        stored.resumeResponseBody = {
          ...
          preview: {
            resume: {
              heading: { name: 'Alex Candidate', contactLine: 'alex@example.com' },
              summary: 'Recovered resume summary.',
              experience: [{ company: 'Example Co', roleTitle: 'Director of Operations', bullets: ['Led operations.'] }],
              education: [],
              competencies: [],
            },
          },
