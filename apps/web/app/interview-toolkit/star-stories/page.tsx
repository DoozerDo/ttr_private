...
export default function StarStoriesPage() {
  const [stories, setStories] = useState<StarStoryDto[]>([]);
  ...
  const [pendingDelete, setPendingDelete] = useState<StarStoryDto | null>(null);
  ...
  const handleEdit = (story: StarStoryDto) => {
  ...
  const validateForm = (state: FormState) => {
    ...
    if (errors.action || errors.result || errors.situation || errors.task || errors.title) {
      return errors;
    }
    return {};
  };
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const payload: StarStoryFormPayload = {
      ...
      reflections: trimmedState.reflections ? trimmedState.reflections : null,
  ...
  const handleDelete = (story: StarStoryDto) => {
    setDeleteError(null);
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ display: "grid", gap: 10 }}>
              ...
              <input
                ...
                onChange={(event) =>
                  setFormState((prev) => ({ ...prev, competenciesInput: event.target.value }))
                }
                style={ttrComponents.input}
