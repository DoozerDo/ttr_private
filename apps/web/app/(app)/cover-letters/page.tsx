...
export default function CoverLettersPage() {
  ...
  const [closingTemplateKey, setClosingTemplateKey] = useState(
    defaultClosingTemplateKey,
  );
    ...
    const loadBaselines = async () => {
      ...
      try {
        ...
        setBaselineId((current) => {
          if (current && data.some((baseline) => baseline.id === current))
            return current;
          return data[0]?.id ?? "";
        });
  ...
  useEffect(() => {
    ...
    setSelectedLetterId((current) => {
      if (current && sortedHistory.some((item) => item.id === current))
        return current;
  ...
  const selectedClosingTemplate = useMemo(
    () =>
      coverLetterClosingTemplates.find(
        (template) => template.key === closingTemplateKey,
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {renderSelect(
              ...
              baselines.map((baseline) => ({
                value: baseline.id,
                label: baseline.originalFilename || "Untitled baseline",
              })),
              setBaselineId,
              baselineState !== "idle",
            )}
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              ...
              >
                <div style={{ ...ttrTypography.subtleLabel, marginBottom: 4 }}>
                  Greeting
                </div>
                <div
                  style={{
            ...
            {statusMessage ? (
              <div style={ttrComponents.successBox}>{statusMessage}</div>
            ) : null}
            {errorMessage ? (
              <div style={ttrComponents.dangerBox}>{errorMessage}</div>
            ) : null}
          </div>
        </section>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            {historyState === "error" ? (
              <div style={ttrComponents.dangerBox}>
                {historyError || "Unable to load history right now."}
              </div>
            ) : null}
