...
export default async function RouteDriftReviewPage() {
  const audit = await runHygieneAudit(process.cwd());
  const summary = summarize(audit.routes.candidates);

  return (
    <div className="space-y-6">
      <header className="space-y-3">
        <p className="text-sm uppercase tracking-[0.4em] text-slate-400">Administrator</p>
        <h1 className="text-3xl font-semibold tracking-tight text-slate-50">Route Drift Review</h1>
        <p className="max-w-3xl text-sm leading-6 text-slate-300">
          Heuristic review only. This surfaces routes that may be drifting out of the canonical product flow.
        </p>
      </header>
