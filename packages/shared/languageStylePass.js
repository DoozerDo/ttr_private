...
exports.buildLanguageStylePass = buildLanguageStylePass;
exports.polishResumeSummaryText = polishResumeSummaryText;
exports.polishResumeBulletsText = polishResumeBulletsText;
exports.polishCoverLetterParagraphsText = polishCoverLetterParagraphsText;
...
function repeatedOpenings(sentences) {
...
function buildIssue(type, severity, location) {
    return { type, severity, location };
}
function polishSummary(summary, input, pass) {
    const sentences = splitSentences(summary);
    if (!sentences.length)
        return summary;
    const leadGeneric = countGenericMatches(sentences[0]) > 0;
    const weakLead = leadGeneric ||
        (0, calibrationFeedback_1.calibrationFeedbackShouldTightenLanguage)(input.feedback) ||
...
function polishBullets(bullets, input, pass) {
    ...
    return bullets.map((bullet, index) => {
        ...
        const verboseThreshold = (0, calibrationFeedback_1.calibrationFeedbackShouldTightenLanguage)(input.feedback) ? 24 : 28;
...
function polishCoverOpening(opening, input, pass) {
    const cleaned = stripGenericPhrases(opening);
    const weakLead = countGenericMatches(opening) > 0 ||
        (0, calibrationFeedback_1.calibrationFeedbackShouldTightenLanguage)(input.feedback) ||
        (0, calibrationFeedback_1.calibrationFeedbackHasAdjustment)(input.feedback, "language_style", ["tighten_language"]) ||
    ...
    const themes = joinThemes([
        ...
        ...input.plan.qualityPass.coverLetterDelta.slice(0, 1),
    ]);
...
function polishCoverParagraphs(paragraphs, input, pass) {
...
function buildLanguageStylePass(input) {
    const pass = {
        issues: [],
    ...
    if ([...resumeBullets, coverOpening, ...coverParagraphs].some((text) => REDUNDANT_MODIFIERS.some((modifier) => new RegExp(`\\b${modifier}\\b`, "i").test(text)))) {
        pass.issues.push(buildIssue("redundant_modifier", "low", "language"));
    }
    if ((0, calibrationFeedback_1.calibrationFeedbackHasAdjustment)(input.feedback, "language_style", ["tighten_language"])) {
        pass.issues.push(buildIssue("generic_phrase", "high", "feedback.language_style"));
    }
    return pass;
}
function polishResumeSummaryText(summary, input, pass) {
    return polishSummary(summary, input, pass);
}
function polishResumeBulletsText(bullets, pass, input) {
...
function polishCoverLetterParagraphsText(paragraphs, input, pass) {
    return polishCoverParagraphs(paragraphs, input, pass);
}
//# sourceMappingURL=languageStylePass.js.map