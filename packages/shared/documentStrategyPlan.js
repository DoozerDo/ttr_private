...
exports.REFINEMENT_PRESETS = void 0;
exports.resolveRefinementTargets = resolveRefinementTargets;
exports.buildDocumentStrategyPlan = buildDocumentStrategyPlan;
exports.buildDocumentStrategyPlanSummary = buildDocumentStrategyPlanSummary;
const calibrationFeedback_1 = require("./calibrationFeedback");
...
function resolveRefinementTargets(instruction) {
...
function calibrationFeedbackHasFocus(feedback, candidate, targetSubsystem) {
    if (!feedback)
        return false;
    const loweredCandidate = normalizeText(candidate).toLowerCase();
    return feedback.adjustments.some((adjustment) => {
        if (targetSubsystem && adjustment.targetSubsystem !== targetSubsystem)
            return false;
...
function countKeywordMatches(corpus, keywords) {
    let total = 0;
...
function extractKeywords(parts, limit = 8) {
    const seen = new Set();
    const result = [];
...
function chooseTitleFamily(corpus) {
...
function chooseSeniority(corpus) {
    if (/\b(vp|vice president|director|head)\b/i.test(corpus))
        return "senior leadership";
    if (/\b(manager|lead)\b/i.test(corpus))
        return "manager level";
    if (/\b(principal|staff|senior)\b/i.test(corpus))
        return "senior individual contributor";
    if (/\b(associate|coordinator|junior)\b/i.test(corpus))
        return "early career";
    return "mid-level";
}
function chooseScope(corpus) {
...
function chooseDomainContext(corpus, jobCompany) {
    if (/\b(saas|software|tech|platform|product)\b/i.test(corpus))
        return "technology and platform operations";
    if (/\b(customer|support|service|cx)\b/i.test(corpus))
        return "customer-facing operations";
...
function choosePriorities(corpus, feedback) {
    const scored = SIGNAL_DEFINITIONS.map((signal) => ({
        label: signal.label,
        score: countKeywordMatches(corpus, signal.keywords) +
            (0, calibrationFeedback_1.calibrationFeedbackBoostForText)(feedback, signal.label, "strategy_plan"),
    }))
...
function chooseRequiredSignals(priorities) {
    return priorities.slice(0, 4);
}
function chooseFitBand(fitScore) {
...
function choosePositioningFrame(corpus, priorities, feedback) {
    ...
    const scored = POSITIONING_FRAME_RULES.map((rule) => ({
        ...
        score: countKeywordMatches(corpus, rule.keywords) +
            (0, calibrationFeedback_1.calibrationFeedbackBoostForText)(feedback, rule.frame, "strategy_plan"),
    })).sort((a, b) => b.score - a.score || a.frame.localeCompare(b.frame));
    return scored[0]?.frame ?? "Operations leader";
}
function buildQualityPass(plan, feedback) {
    const topEvidence = plan.selectedEvidence.slice(0, 3);
    const prioritizedAxes = [...plan.roleLens.priorities].sort((a, b) => (0, calibrationFeedback_1.calibrationFeedbackBoostForText)(feedback, b, "refinement_bias") -
        (0, calibrationFeedback_1.calibrationFeedbackBoostForText)(feedback, a, "refinement_bias") ||
        (0, calibrationFeedback_1.calibrationFeedbackBoostForText)(feedback, b, "strategy_plan") -
            (0, calibrationFeedback_1.calibrationFeedbackBoostForText)(feedback, a, "strategy_plan"));
    const topNarrativeAxes = Array.from(new Set([
        ...prioritizedAxes.slice(0, 3),
        ...topEvidence.flatMap((evidence) => evidence.matchedSignals.slice(0, 2)),
    ])).slice(0, 4);
    const framingStrength = plan.fitBand === "strong" && topEvidence.length >= 2 ? "high" : plan.fitBand === "borderline" ? "low" : "medium";
    const emphasisConfidence = topEvidence.length >= 3 && topNarrativeAxes.length >= 2 ? "high" : topEvidence.length >= 2 ? "medium" : "low";
    const cutCandidates = Array.from(new Set(plan.suppressionNotes
        .flatMap((note) => note.split(/[.;]/))
        .map((value) => normalizeText(value))
        .filter((value) => value.length > 0)
        .filter((value) => /lower-relevance|weaker overlap|opening frame|background/i.test(value)))).slice(0, (0, calibrationFeedback_1.calibrationFeedbackHasAdjustment)(feedback, "quality_pass", ["suppress_signal"]) ? 5 : 4);
    const mustLeadWith = Array.from(new Set([
        plan.positioningFrame,
        ...(topNarrativeAxes[0] ? [topNarrativeAxes[0]] : []),
        ...(topEvidence[0]?.baselineSection ? [topEvidence[0].baselineSection] : []),
        ...((0, calibrationFeedback_1.calibrationFeedbackHasAdjustment)(feedback, "strategy_plan", ["promote_signal"])
            ? [topNarrativeAxes[0] ?? plan.positioningFrame]
            : []),
    ])).filter(Boolean).slice(0, 3);
    const avoidRepeating = Array.from(new Set([
        ...GENERIC_FILLER_PHRASES,
        ...GENERIC_RESUME_PHRASES,
        ...plan.roleLens.priorities.slice(1, 4),
    ])).slice(0, 8);
    const coverLetterDelta = Array.from(new Set([
        ...
        ...((0, calibrationFeedback_1.calibrationFeedbackHasAdjustment)(feedback, "quality_pass", ["increase_weight"])
...
function computeDocumentQualityScore(plan) {
    const framingScore = plan.qualityPass.framingStrength === "high"
        ? 30
        : plan.qualityPass.framingStrength === "medium"
            ? 20
            : 10;
    const emphasisScore = plan.qualityPass.emphasisConfidence === "high"
        ? 25
    ...
    const bandScore = plan.fitBand === "strong" ? 5 : plan.fitBand === "moderate" ? 3 : plan.fitBand === "borderline" ? 1 : 2;
    return Math.max(0, Math.min(100, framingScore + emphasisScore + evidenceScore + suppressionScore + precisionScore + bandScore));
}
function cleanSectionLabel(value, sectionType) {
    const label = normalizeText(value ?? "");
    if (label)
        return label;
    if (sectionType)
        return sectionType.replace(/_/g, " ").toLowerCase();
    return "Baseline evidence";
}
function splitClaims(content) {
    return content
        .split(/\r?\n|(?<=[.!?])\s+/)
...
function chooseApprovedClaims(content, matchedSignals) {
    const claims = splitClaims(content);
    const signalKeywords = matchedSignals.flatMap((signal) => SIGNAL_DEFINITIONS.find((definition) => definition.label === signal)?.keywords ?? []);
    const matchedClaims = claims.filter((claim) => signalKeywords.some((keyword) => claim.toLowerCase().includes(keyword.toLowerCase())));
    if (matchedClaims.length)
        return matchedClaims.slice(0, 3);
    const fallback = claims.find((claim) => claim.length > 0);
    return fallback ? [fallback] : [];
}
function buildMatchedSignals(sectionText) {
    return SIGNAL_DEFINITIONS.filter((signal) => signal.keywords.some((keyword) => sectionText.includes(keyword.toLowerCase())))
        .map((signal) => signal.label)
        .slice(0, 4);
}
function scoreSection(sectionText, corpus, priorities, feedback) {
    ...
    const outcomeSignals = /\b(led|built|owned|improved|delivered|implemented|scaled|reduced|increased|launched|designed|managed)\b/i.test(sectionText)
        ? 3
        : 0;
    const quantifiedSignals = /\b\d+(?:%|x|k|m|million|billion)?\b/i.test(sectionText) ? 3 : 0;
    const strategicSignals = /\b(strategy|program|architecture|workflow|process|operational|cross-functional)\b/i.test(sectionText)
        ? 2
        : 0;
    const feedbackBoost = (0, calibrationFeedback_1.calibrationFeedbackBoostForText)(feedback, sectionText, "strategy_plan") +
        (0, calibrationFeedback_1.calibrationFeedbackBoostForText)(feedback, sectionText, "refinement_bias");
    const feedbackPenalty = (0, calibrationFeedback_1.calibrationFeedbackHasAdjustment)(feedback, "strategy_plan", ["suppress_signal"]) &&
...
function buildEvidenceSelection(sections, corpus, priorities, feedback) {
    ...
    const minimumScore = (0, calibrationFeedback_1.calibrationFeedbackHasAdjustment)(feedback, "strategy_plan", ["suppress_signal"]) ? 2 : 0;
    const selectedLimit = (0, calibrationFeedback_1.calibrationFeedbackHasAdjustment)(feedback, "refinement_bias", ["promote_signal", "increase_weight"]) ? 3 : 4;
    ...
    const selectedEvidence = selected.map(({ section, sectionText, index }, rank) => {
        ...
        const whySelected = matchedSignals.length > 0
    ...
    const suppressionNotes = [
        ...
        ...((0, calibrationFeedback_1.calibrationFeedbackHasAdjustment)(feedback, "strategy_plan", ["suppress_signal"])
            ? ["Feedback tightened the evidence bar so weaker background stays suppressed."]
            : []),
    ];
    return { selectedEvidence, suppressionNotes };
}
function buildSummaryStrategy(positioningFrame, priorities, fitBand) {
...
function buildResumeEmphasis(priorities, selectedEvidence) {
    const emphasis = [...priorities.slice(0, 3)];
    for (const evidence of selectedEvidence) {
        if (emphasis.length >= 4)
            break;
...
function buildCoverLetterThemes(positioningFrame, priorities, selectedEvidence) {
    const themes = [
...
function normalizeRefinementText(value) {
    return normalizeText(value).toLowerCase();
}
function refinementFocusLabels(instruction) {
...
function refinementFocusTerms(instruction) {
    const text = normalizeRefinementText(instruction.instruction);
    const terms = [];
    const maybeAdd = (term) => {
...
function scoreRefinedEvidenceEntry(entry, focusTerms) {
    const corpus = [
    ...
    if (focusTerms.includes("leadership") && /lead|direct|manage|owner|head/i.test(corpus))
        score += 4;
    if (focusTerms.includes("operations") && /support|service|ops|process|workflow/i.test(corpus))
        score += 4;
    if (focusTerms.includes("execution") && /deliver|implement|launch|scale|reduce|increase/i.test(corpus))
        score += 4;
    return score;
}
function rerankEvidenceForRefinement(_plan, instruction, selectedEvidence) {
    if (instruction.constraints.preserveSelectedEvidence) {
        const focusTerms = refinementFocusTerms(instruction);
        return [...selectedEvidence]
            .sort((a, b) => scoreRefinedEvidenceEntry(b, focusTerms) -
    ...
    const reordered = [...evidence].sort((a, b) => scoreRefinedEvidenceEntry(b, focusTerms) - scoreRefinedEvidenceEntry(a, focusTerms) ||
        Number(a.rank ?? 0) - Number(b.rank ?? 0));
    return reordered.map((entry, index) => ({ ...entry, rank: index + 1 }));
}
function buildRefinedSummaryStrategy(plan, instruction, activeEvidence) {
    const focusLabels = refinementFocusLabels(instruction);
...
function buildRefinedResumeEmphasis(baseEmphasis, instruction, activeEvidence) {
    ...
    for (const evidence of activeEvidence.slice(0, 2)) {
        if (!next.includes(evidence.baselineSection))
            next.push(evidence.baselineSection);
    }
    return next.slice(0, 4);
}
function buildRefinedCoverLetterThemes(baseThemes, instruction, activeEvidence) {
...
function buildRefinedQualityPass(basePlan, activeEvidence, instruction, suppressionNotes) {
    ...
    if (instruction.type === "summary_rewrite" || instruction.type === "bullet_focus") {
        next.mustLeadWith = Array.from(new Set([
            ...next.mustLeadWith,
            activeEvidence[0]?.baselineSection ?? basePlan.positioningFrame,
        ])).slice(0, 3);
    }
    if (instruction.type === "cover_letter_focus") {
        next.coverLetterDelta = Array.from(new Set([
...
function validateRefinedPlan(basePlan, refinedPlan, instruction) {
    const reasons = [];
...
function applyRefinementInstructions(input, basePlan, refinements) {
    ...
    for (const instruction of refinements) {
        ...
        const nextSelectedEvidence = rerankEvidenceForRefinement(nextPlan, instruction, nextPlan.selectedEvidence);
        const shouldRebuildEvidence = instruction.type === "evidence_swap" &&
            instruction.constraints.allowNewEvidenceFromBaseline &&
            Array.isArray(input.baselineSections) &&
            input.baselineSections.length > 0;
        const nextRolePriorities = Array.from(new Set([
        ...
        if (instruction.type === "emphasis_shift" || instruction.type === "tone_adjustment") {
            ...
            nextPlan.coverLetterThemes = buildRefinedCoverLetterThemes(nextPlan.coverLetterThemes, instruction, nextPlan.selectedEvidence);
        }
        if (instruction.type === "summary_rewrite") {
            nextPlan.summaryStrategy = buildRefinedSummaryStrategy(nextPlan, instruction, nextPlan.selectedEvidence);
            nextPlan.resumeEmphasis = buildRefinedResumeEmphasis(nextPlan.resumeEmphasis, instruction, nextPlan.selectedEvidence);
        ...
        nextPlan.suppressionNotes = Array.from(new Set([
        ...
        nextPlan.qualityPass = buildRefinedQualityPass(nextPlan, nextPlan.selectedEvidence, instruction, nextPlan.suppressionNotes);
...
function buildDocumentStrategyPlan(input, feedback) {
...
function buildDocumentStrategyPlanSummary(plan) {
    return {
...
//# sourceMappingURL=documentStrategyPlan.js.map