...
exports.buildCalibrationFeedback = buildCalibrationFeedback;
exports.calibrationFeedbackHasAdjustment = calibrationFeedbackHasAdjustment;
exports.calibrationFeedbackBoostForText = calibrationFeedbackBoostForText;
exports.calibrationFeedbackShouldTightenLanguage = calibrationFeedbackShouldTightenLanguage;
function normalizeText(value) {
    return value.replace(/\s+/g, " ").trim().toLowerCase();
}
function uniqueAdjustments(adjustments) {
    const seen = new Set();
    const result = [];
...
function gapStrength(severity) {
    return severity;
}
function gapToAdjustments(gap, plan) {
...
function dimensionBasedFallbacks(calibration, plan) {
    const adjustments = [];
    const add = (targetSubsystem, action, focus, strength) => {
...
function buildCalibrationFeedback(input) {
    const adjustments = [
...
function calibrationFeedbackHasAdjustment(feedback, targetSubsystem, actions) {
    if (!feedback)
        return false;
    return feedback.adjustments.some((adjustment) => {
        if (adjustment.targetSubsystem !== targetSubsystem)
            return false;
...
function calibrationFeedbackBoostForText(feedback, text, targetSubsystem) {
    if (!feedback)
        return 0;
...
function calibrationFeedbackShouldTightenLanguage(feedback) {
...
//# sourceMappingURL=calibrationFeedback.js.map